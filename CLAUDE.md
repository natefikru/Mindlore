...
`.github/workflows/ci.yml` builds once (`scripts/ci/build-for-testing.sh`) and tests from those products: `unit` on every pull request and push to `main` (the only required check), and `ui` split across five runners on every push to `main`, on a PR carrying the `ui` label, or on manual dispatch. `ui` waits for `unit` to finish, pass or fail, so the required check never queues behind five advisory shards on the free plan's five-runner cap. A manual run with the `only` input runs just the named UI tests (`Class` or `Class/testMethod`) on one runner. On CI a failed UI test is retried once, because shared runners sometimes fail to terminate or launch the app between tests. Before the unit run, `test.sh unit` runs `LanguageAssetsWarmUp` on its own, which waits for NLTagger's English lemma assets; without them a fresh runner fails every lemma test. `scripts/ci/ui-shards.sh` computes the split from the files in `MindloreUITests/`, weighted by test count, so a new class needs nothing registered; the class name must match the file name. `scripts/ci/test.sh unit` and `scripts/ci/test.sh ui <Class> ...` are the same commands locally (they build first if `build/DerivedData` is empty). Signing stays at the project's defaults: `CODE_SIGNING_ALLOWED=NO` fails `KeychainSecretStoreTests` with `errSecMissingEntitlement`. The scheme is shared (`xcshareddata/xcschemes/Mindlore.xcscheme`); keep it that way or CI has nothing to build. `release.yml` archives and uploads to TestFlight and is gated off by the `RELEASE_ENABLED` repository variable until the paid team exists; the placeholders are listed at the top of that file and in `docs/remaining-work.md`. The plan and its measurements are in `tasks/ci-plan.md`.
