...
- **Mind** (`Views/Mind/`) is the second tab: one full-screen map where each channel means one
  ...
  both schemes; a tag is a 2pt dark-neutral pin (`GraphSimulation.tagRadius`, one size at any
...
- **No advice, same rule as Ask and loose ends.** The narrative states what the numbers show and
  nothing else.
- **Loose ends tab** (owner, 2026-09-24). A segmented control in the sheet's bar switches between
  Summaries (the feed, the default) and Loose ends (`ReflectLooseEndsView`): every `LooseEnd` in any
  status, newest first by `sourceEntryDate` (the raising entry's journal day, so closing or
  reopening never moves a row), grouped by month, with All / Open / Closed chips.
