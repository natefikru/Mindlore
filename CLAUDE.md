...
The app signs with team `7DZBU56KUA`, a paid Individual team since 2026-09-23 (it was the free Personal Team before, same ID). The app's own journal mirrors to `iCloud.com.natefikru.mindlore` (`AppConfig.cloudKitContainerID`) through SwiftData's CloudKit store; only `StoreLocation.default` does, so tests and demo journals never touch iCloud. A store that fails to open with CloudKit reopens on the device alone (`sync.storeFailed`). `SyncStatusMonitor` (`Mindlore/Sync/`) reports the account and the mirroring's own events to the iCloud section at the top of Settings' General screen; `SyncStatus` holds every sentence it can say. Xcode builds talk to CloudKit's Development environment and TestFlight to Production, and nothing copies data between them. Production only accepts fields its schema already has, and Development only learns a field once a record carrying it syncs, so after any model change: launch a Debug build on a signed-in phone with `-initializeCloudKitSchema` (`scripts/device/launch.sh schema -- -initializeCloudKitSchema`, `CloudKitSchemaInitializer`), which writes every field to Development; check it with `xcrun cktool export-schema --team-id 7DZBU56KUA --container-id iCloud.com.natefikru.mindlore --environment development` (a management token saved with `cktool save-token --type management`); then press Deploy Schema Changes in CloudKit Console, which no API can do (cktool answers "endpoint not applicable in the environment 'production'"). Do it before the merge whose TestFlight build writes the new field. Core Data's mirroring deletes synced data on sign-out or an account change, so every save also writes the entry's words to `Application Support/EntryBackups/` (`EntryBackups`, registered to the journal's container only, so tests never write there), and `JournalRecovery` offers them back through `RestoreBanner` once sync settles after such an event. A save path that bypasses `saveStampingEntries` and `EntrySaver` skips the copy until the next launch fills it in. Automatic AI runs only on the phone that made the entry: `LocalOrigin` (`Mindlore/Sync/`) keeps those entry ids in UserDefaults, claimed by the two save paths from the context's inserted entries (CloudKit's imports never pass through them) and seeded once from the whole store, and the transcription, page, title, and insights queues and `AIPassTrigger` skip any other entry, because the pending flags sync. The manual paths (Retry, Transcribe here, Run AI, Redo insights) run anywhere and claim the entry. Like `EntryBackups`, it is registered to the journal's container only, so a test or demo store treats every entry as local. `SyncDuplicates` folds what two phones made apart, at launch after the graph sweep and each time sync settles: untouched entities with the same key and kind (earliest `createdAt`, then id, through `GraphEditor.merge(byUser: false)`, which claims nothing), recaps by period (Life's feedback verdicts unioned), and, on the entry's own phone only since they have no id to agree on, doubled links and insights rows. `AskMessage.all` orders by index then id and `AskService` renumbers a repeated index. `LaunchSweepIdempotenceTests` runs every launch sweep twice and requires identical records. Sync with iCloud is a switch at the top of General (`SyncSwitch`, this phone's own `UserDefaults`, on by default). Mirroring can't be turned off on a live container, so `JournalHost` (`Mindlore/Persistence/`) owns opening the store, not `MindloreApp.init`: the switch saves the choice at once and `SettingsView`'s `onDisappear` applies it, dropping the root for `SyncSwitchingView`, waiting up to five seconds for the old container to be released (`sync.switched` says whether it was), and opening a new one; `RootView` is keyed on the host's generation, so every coordinator starts afresh. `EntryBackups`, `LocalOrigin`, and `JournalRecovery` follow "is the journal", not "syncs now", so with sync off another phone's entries still wait for it. The journal's own settings follow it to the user's other phones through `MirroredKeyValueStore` (`NSUbiquitousKeyValueStore` behind `CloudKeyValues`) for the keys in `SettingsStore.Key.mirrored` (areas, name, voice, font, insight sections and prompts, cleanup and date behaviour, keep recordings, resurfacing); everything about the phone (appearance, the lock, the reminder, AI and its accounts, record on open) stays on it. Reads are always local; at launch the cloud wins where it has a value and what only this phone has goes up, and a key nobody set stays missing on both sides. `SettingsStore.reloadMirrored` copies incoming values through a fresh store so defaults live only in `init`, and `mirroredCopies` must list every mirrored key. The switch stops settings mirroring too. The plan and its remaining phases are `tasks/icloud-sync.md`.
