...
`.github/workflows/ci.yml` runs five jobs at once, the free plan's limit of concurrent macOS runners: `unit` on every pull request and push to `main` (the only required check), and `ui` split four ways on every push to `main`, on a PR carrying the `ui` label, or on manual dispatch. Each job builds for itself (`scripts/ci/build-for-testing.sh`) after starting its simulator's boot in the background (`scripts/ci/boot-simulator.sh --background`), so the minutes a fresh runner spends booting hide inside the compile; a shared build job put its build in front of every boot instead. The shards are `continue-on-error`: a UI failure shows on its own job but never sets the run's result. A manual run with the `only` input runs just the named UI tests (`Class` or `Class/testMethod`) on one runner. On CI a failed UI test is retried once, because shared runners sometimes fail to terminate or launch the app between tests. Runners have no NLTagger lemma assets and `NLTagger.requestAssets` hangs there, so tests that depend on lemmas are `.enabled(if: LemmaAvailability.isAvailable)`: they run on every Mac and phone and show as skipped on CI. `scripts/ci/ui-shards.sh` computes the split from the files in `MindloreUITests/`, weighted by test count, so a new class needs nothing registered; the class name must match the file name. `scripts/ci/test.sh unit` and `scripts/ci/test.sh ui <Class> ...` are the same commands locally (they build first if nothing is built; local products go to `~/Library/Developer/Xcode/DerivedData/Mindlore-ci`, since builds under `~/Documents` fail signing). Signing stays at the project's defaults: `CODE_SIGNING_ALLOWED=NO` fails `KeychainSecretStoreTests` with `errSecMissingEntitlement`. The scheme is shared (`xcshareddata/xcschemes/Mindlore.xcscheme`); keep it that way or CI has nothing to build. `release.yml` archives and uploads to TestFlight and is gated off by the `RELEASE_ENABLED` repository variable until the paid team exists; the placeholders are listed at the top of that file and in `docs/remaining-work.md`. The plan and its measurements are in `tasks/ci-plan.md`.
