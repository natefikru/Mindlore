...
// 200 of them out of the type checker. The text, dates, and titles are hand-written; the insights,
// parts, and loose ends each entry carries were generated by the app's own pipeline
// (scripts/demo/regenerate-story.sh), so seeding needs no AI. Dates move forward by whole days to end on the day it's seeded, so the
