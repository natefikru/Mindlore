...
struct GraphSimulationTests {
    ...
    @Test func updateKeepsAnAnchorAnchored() {
        let nodes = makeNodes(3)
        let simulation = GraphSimulation(nodes: nodes, edges: [])
        simulation.anchor(nodes[0].id, at: .zero)
        simulation.update(nodes: nodes + makeNodes(2), edges: [EntityGraph.Edge(nodes[0].id, nodes[1].id, weight: 1)])
        for _ in 0..<100 { simulation.tick() }
        #expect(simulation.position(of: nodes[0].id) == .zero)
        simulation.unpin(nodes[0].id)
        #expect(simulation.isPinned(nodes[0].id))
    }
