...
struct GraphEditorTests {
    ...
    // Hiding is the user's, so it outlives losing every link.
    @Test func hidingKeepsTheEntityAlive() throws {
        let entry = try harness.entry(tags: ["monday"])
        harness.indexer.sweep(in: harness.context)
        let monday = try harness.entity("monday")

        editor.setHidden(true, on: monday)
        ...
        #expect(try harness.entity("monday").hidden)
    }
