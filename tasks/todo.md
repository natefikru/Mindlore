...
- [x] `Mindlore/Transcription/Transcriber.swift`: `protocol Transcriber { func transcribe(audioFileURL: URL, locale: Locale) async throws -> String }` and `TranscriptionError` (`authorizationDenied` and `unsupportedLocale` are permanent; `assetsUnavailable` and `analysisFailed` can be retried), each with a plain-language message.
- [x] `Mindlore/Transcription/SpeechAnalyzerTranscriber.swift`: request speech recognition authorization before the first transcription. Pick `SpeechTranscriber` if `isAvailable` and it supports the locale, else `DictationTranscriber` (which has no availability check in the SDK) if it supports the locale, else throw `unsupportedLocale`. The work runs `@concurrent`, off the main actor. Resolve the locale with `supportedLocale(equivalentTo:)`. Call `AssetInventory.assetInstallationRequest(supporting:)`; `nil` means assets are installed, otherwise `downloadAndInstall()`. Start a task reading `results` before `analyzer.start(inputAudioFile:finishAfterFile: true)`, then await it and join finalized results. Distinct errors for denied authorization, unsupported locale, asset download failure, analysis failure.
- [x] `Mindlore/Transcription/TranscriptionCoordinator.swift`: `@Observable`, tracks in-progress `PersistentIdentifier`s and per-entry errors. `processQueue(context:)` fetches entries with `awaitingText == true`, one at a time: write audio to a temp file, transcribe, re-fetch by identifier after the `await`. If the entry is gone, discard the result. Otherwise call `applyGeneratedText(_:)` (which refuses if the user typed in the meantime) and save immediately. On failure the flag stays set and the error is stored in memory; that entry is skipped for the rest of the session until the user taps Retry, and tried again on the next launch. A call to `processQueue` while it is already running schedules one more pass, so an entry recorded mid-run is picked up. Audio that couldn't be converted at ingest is handed over as `.caf`, converted audio as `.m4a`. Called at launch after ingest and after each new ingest.
- [x] Editor states for voice entries with `awaitingText`: in progress (progress, and the text field stays editable; typing clears the flag and the pending result is dropped); failed (reason, "Retry"); permanent error (explanation, no retry). On close, `Entry.discardAudioIfNotKept(keepAudio:)` removes the recording when the setting is off and the text was generated. The "Type instead" button goes away because typing is always available.
